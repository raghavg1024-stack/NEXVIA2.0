...
export function WaitlistForm() {
  ...
  return <form action={action} className="space-y-4"><input name="website" tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true" /><label className="block text-sm">Full name<input required name="full_name" className={`${fieldClass} mt-2`} /></label><label className="block text-sm">Email<input required type="email" name="email" className={`${fieldClass} mt-2`} /></label><label className="block text-sm">I am a<select required name="role" className={`${fieldClass} mt-2`} defaultValue="student"><option className="bg-slate-950" value="student">Student</option><option className="bg-slate-950" value="parent">Parent</option><option className="bg-slate-950" value="academia">Academic partner</option><option className="bg-slate-950" value="industry">Industry partner</option><option className="bg-slate-950" value="other">Other</option></select></label><label className="flex gap-3 text-sm text-slate-400"><input required type="checkbox" name="consent" className="mt-1" /> I agree to be contacted about Nexvia access.</label><Status error={state.error} pending={pending} /></form>;
}

export function ContactForm() {
  const [state, action, pending] = useActionState(sendContactMessage, initialState);
  return <form action={action} className="space-y-4"><input name="website" tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true" /><label className="block text-sm">Name<input required name="name" className={`${fieldClass} mt-2`} /></label><label className="block text-sm">Email<input required type="email" name="email" className={`${fieldClass} mt-2`} /></label><label className="block text-sm">Subject<input required name="subject" className={`${fieldClass} mt-2`} /></label><label className="block text-sm">Message<textarea required name="message" rows={6} className={`${fieldClass} mt-2 resize-y`} /></label><Status error={state.error} pending={pending} /></form>;
}
