      ...
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col gap-5 border-b border-white/10 pb-7 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">A complete sample journey showing how one student moves from profile signals to career evidence and opportunities.</p>
          </div>
        ...
        <section className="mt-8 grid gap-5 lg:grid-cols-[.78fr_1.22fr]">
          ...
          <article className="rounded-3xl border border-cyan-400/20 bg-card p-6">
        ...
        <section className="mt-5 rounded-3xl border border-white/10 bg-card p-6">
        ...
        <section className="mt-5 grid gap-5 md:grid-cols-3">
          {[{ icon: Brain, title: "AI Mentor", text: "Explains the next action using the learner’s selected career, skill gaps and progress." }, { icon: Mic2, title: "Mock Interview", text: "Scores structured answers consistently, then uses Gemini for specific coaching." }, { icon: BriefcaseBusiness, title: "Opportunity Match", text: "Connects the roadmap to relevant jobs and shows nearby alternatives when exact matches are unavailable." }].map(({ icon: Icon, title, text }) => <article key={title} className="rounded-2xl border border-line bg-card p-5"><Icon className="h-5 w-5 text-cyan-300" /><h2 className="mt-4 font-bold text-white">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{text}</p></article>)}
        </section>

        <section className="mt-5 rounded-3xl border border-emerald-400/20 bg-emerald-400/5 p-6 sm:p-8">
          <div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-center"><div><div className="flex items-center gap-2 text-emerald-300"><CheckCircle2 className="h-5 w-5" /><span className="text-xs font-bold uppercase tracking-[0.16em]">Measurable outcome</span></div><h2 className="mt-3 font-display text-2xl uppercase text-white">From uncertain to application-ready</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">The system tracks roadmap completion, skill evidence, interview improvement and application activity. Pilot validation is still required before claiming a final accuracy percentage.</p></div><Link href="/signup" className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-slate-950 transition hover:bg-cyan-100">Try Nexvia <ArrowRight className="h-4 w-4" /></Link></div>
        </section>
